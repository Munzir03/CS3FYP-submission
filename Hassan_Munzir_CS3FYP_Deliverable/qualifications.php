...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['university']) && !empty($_POST['qualification']) && !empty($_POST['grade'])) {
        ...
        $stmt = $con->prepare('INSERT INTO qualifications (university, qualification, grade, accounts_id) VALUES (?, ?, ?, ?)');
        // Bind parameters to the query
        $stmt->bind_param('sssi', $_POST['university'], $_POST['qualification'], $_POST['grade'], $accounts_id);

        // Execute the statement
        if ($stmt->execute()) {
            echo 'Success: The qualification has been added.';
        } else {
    ...
    $stmt = $con->prepare('SELECT * FROM qualifications WHERE accounts_id = ? ORDER BY date_added DESC LIMIT 5');
